...
class DefaultSessionHandlerTest extends TestCase
{
    public function testNativeInteroperability()
    {
        ...
        $this->assertEquals('test|s:4:"1234";', $file->read());
    }
    ...
    public function testNormalBehavior()
    {
        ...
        $file = $session->getSessionFile();
        $this->assertNotNull($file);
        $this->assertEquals('val|s:4:"4567";val2|s:3:"999";val3|s:4:"2345";test|s:4:"allo";', $file->read());
    }
    ...
    public function testSavePathNotExist()
    {
        $path = ROOT_DIR . '/toto';
        $this->expectException(SessionPathNotExistException::class);
        $this->expectExceptionMessage("ZEPHYRUS SESSION: The specified session save path [$path] doesn't exist.");
        $session = new Session([
            'storage' => 'file',
            'save_path' => $path
        ]);
    ...
    public function testInvalidStorageMode()
    {
        $this->expectException(SessionStorageModeException::class);
        $this->expectExceptionMessage("ZEPHYRUS SESSION: Session storage configuration property must be either file or database.");
        new Session([
            'storage' => 'wrong'
        ]);
    }
    ...
    {
        $this->expectException(SessionLifetimeException::class);
        $this->expectExceptionMessage("ZEPHYRUS SESSION: Session lifetime configuration property must be int (seconds before expiration, defaults to 1440).");
        new Session([
            'lifetime' => 'wrong'
        ]);
    }
