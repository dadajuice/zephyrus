...
class LimitClauseTest extends TestCase
{
    public function testLimit()
    {
        ...
        self::assertEquals("LIMIT 50", $limit->getSql(new SqliteAdapter(new DatabaseSource())));
    }

    public function testLimitWithOffsetSqlite()
    {
        $limit = new LimitClause(50, 10);
        self::assertEquals("LIMIT 10, 50", $limit->getSql(new SqliteAdapter(new DatabaseSource())));
    }

    public function testLimitWithOffsetMysql()
    {
        $limit = new LimitClause(50, 10);
        self::assertEquals("LIMIT 10, 50", $limit->getSql(new MysqlAdapter(new DatabaseSource())));
    }

    public function testLimitWithOffsetPostgres()
    {
        $limit = new LimitClause(50, 10);
        self::assertEquals("LIMIT 50 OFFSET 10", $limit->getSql(new PostgresAdapter(new DatabaseSource())));
    }
}
